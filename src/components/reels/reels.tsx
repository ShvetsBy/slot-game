...
// type cardsImgRecord = "ACard" | "JCard" | "KCard" | "QCard"  | "Flamingo" | "Plane" | "FreeSpins" | "CatchMe" | "Gun" | "WalkieTalkie" | "WildCard" | "WildFire" | "Jerk" | "Girl"
...
export function Reels() {
  ...
  const getShuffled = (arr: number[]) => {
    const shuffledArr = arr.map((value) => (
  ...
  const click = () => {
    ...
    const getResult = () => {
      ...
      return result;
        ...
        type findInArrayType = (array: Array<ReelSymbol>, item: ReelSymbol, index: number, result: ReelSymbol[]) => void;

        const findInArray: findInArrayType = (array, item, index, result) => {
          let currentIndex: number = index;
          const matchItem: ReelSymbol = array[currentIndex].find((el: ReelSymbol) => el.name === item.name);
          if (matchItem) {
  ...
  function ReelContainer(props: { x: number; data: ReelSymbol[]; }) {
    ...
    function WinMsg({ text }: WinMsgProp) {
      return (
        <Text
          text={text}
          anchor={0.5}
    ...
    function ReelsContainer() {
      ...
      if (allReelData.length) {
        return (
          <>
            {row.map((el, i) => <ReelContainer x={el.x} data={allReelData[i]} key={i} />)}
    ...
    function ReelsContainerWrapper() {
      return (
